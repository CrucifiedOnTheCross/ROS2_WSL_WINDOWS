...
## Как показала практика, у многих Windows не поддерживает зи пакета XServer, чтобы с этим не мучаться, вы можете скачать `MobaXterm`
[MobaXterm](https://mobaxterm.mobatek.net/) CСсылка на скачивание

![alt text](./img/MobaXterm_Select.png)

Слева у вас должен появится панель выбора удаленных хостов, среди них находится и WSL

Давайте запустим.

![alt text](./img/CONSOLE.png)

Теперь перед нами появляется файловая структура с консолью, мы можем быстро и оперативно перемешаться среди папок, редактировать и удалять файлы

Также есть встроеенная поддержка XServer, которая позволит запускать графические приложения